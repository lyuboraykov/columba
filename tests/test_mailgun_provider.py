...
TEST_DOMAIN = 'sandbox5faa34abe01446d5be16abc8077cb531.mailgun.org'
TEST_AUTHENTICATION = 'key-b5b8a218881f2d7c34b7fb14fe5deb0f'

def test_mailgun_send_positive():
    """Straightforward mailgun send test. Will fail if an error is raised by the provider."""
    mailgun_provider = MailGunProvider(TEST_AUTHENTICATION, TEST_DOMAIN)
...
def test_sendgrid_send_wrong_authentication():
    ...
    mailgun_provider = MailGunProvider(TEST_AUTHENTICATION + 's', TEST_DOMAIN)
    test_message = init_test_objects.init_message()
